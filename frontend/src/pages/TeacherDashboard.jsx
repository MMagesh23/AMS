...
const TeacherDashboard = () => {
  ...
  }, [timeWindow]); // Recalculate whenever the timeWindow state changes
  ...
  useEffect(() => {
    const fetchTimeWindow = async () => {
      try {
        const res = await api.get('/teacher/time-window');
        setTimeWindow(res.data); // Update the state with the fetched time window
        console.log('Time window fetched:', res.data); // Debugging log
      } catch (err) {
    ...
    fetchTimeWindow();
  }, []); // Ensure this runs when the component loads
    ...
    try {
      ...
      if (timeWindow) {
        ...
        if (now < startTime || now > endTime) {
          toast.error(
            `Attendance can only be submitted between ${timeWindow.startTime} and ${timeWindow.endTime}`
          );
          return;
        }
      }
  
      // Transform attendance object into an array of records
      ...
      const response = await api.post(
        ...
        { records }, // Send records in the request body
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
      ...
      setAttendanceStatus(true); // Mark attendance as submitted
    } catch (err) {
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
                ...
                <tbody>
                  {classInfo.students.map((student, index) => (
                    <tr key={student._id} className="border-t">
                      ...
                      <td className="p-2">
                          <label className="mr-4 text-green-600 font-medium">
                          <input
                            type="radio"
                            ...
                            disabled={attendanceStatus || !timeRemaining} // Disable if attendance is submitted or time window is closed
                            className="mr-1 "
                          />
                             Present
                          </label>
                          <label className="mr-4 text-red-600 font-medium">
                          <input
                            type="radio"
                            name={`status-${student._id}`}
                            value="absent"
        ...
        <div className="bg-green-50 p-4 rounded-xl shadow-md mb-6">
          ...
          <button
            ...
            className={`w-full bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 ${
              isSubmitting || attendanceStatus ? "opacity-50 cursor-not-allowed" : ""
            }`}
      ...
      {loadingAttendanceRecords ? (
        ...
        <div className="mt-8">
          <h2 className="text-lg font-semibold mb-4">📅 Past Attendance</h2>
