  ...
  const renderStudentsByGrade = (group) => {
    ...
    return (
      <div key={group} className="mb-8">
        <h3 className="text-2xl font-bold text-indigo-700 mb-4 border-b pb-1 border-indigo-300">{group}</h3>
        {Object.keys(groupedByGrade)
          .sort((a, b) => a - b)
          .map((gradeKey) => (
            <div key={gradeKey} className="mb-6">
              <h4 className="text-lg font-semibold text-gray-700 mb-2">{gradeToLabel(parseInt(gradeKey))}</h4>
                  ...
                  <tbody>
                    {groupedByGrade[gradeKey].map((student, index) => (
                      <tr key={student._id} className="hover:bg-indigo-50">
                        ...
                        <td className="border px-3 py-2">{gradeToLabel(student.grade)}</td>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6 flex flex-col md:flex-row gap-4 items-center">
          <input
            ...
            className="px-4 py-2 w-full md:w-1/2 border rounded-lg shadow-sm"
            value={searchTerm}
